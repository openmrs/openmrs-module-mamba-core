...
public class MambaReportResource implements Searchable {
					...
					try {
						...
						if (pageNumber < 1) {
							return RestUtil.wrapErrorResponse(
								new IllegalArgumentException("page_number must be positive"),
								"Invalid page_number: must be >= 1"
							);
						}
						searchCriteria.setPageNumber(pageNumber);
					} catch (NumberFormatException e) {
						return RestUtil.wrapErrorResponse(e, "Invalid page_number: must be a number");
					}
					break;
				case "page_size":
					try {
						...
						if (pageSize <= 0) {
							return RestUtil.wrapErrorResponse(
								new IllegalArgumentException("page_size must be positive"),
								"Invalid page_size: must be > 0"
							);
						}
						searchCriteria.setPageSize(pageSize);
					} catch (NumberFormatException e) {
						return RestUtil.wrapErrorResponse(e, "Invalid page_size: must be a number");
					}
					break;
				default:
	...
	public String getUri(Object o) {
		// This is a search-based resource - individual items don't have direct URIs
		// Clients should search by report_id parameter
		return null;
	}
}
