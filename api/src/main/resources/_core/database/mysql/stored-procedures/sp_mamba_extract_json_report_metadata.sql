...
BEGIN
    ...
    SELECT JSON_EXTRACT(@report, '$.report_name') INTO @report_name FROM mamba_dim_json;
    SELECT JSON_EXTRACT(@report, '$.flat_table_name') INTO @flat_table_name FROM mamba_dim_json;
    SELECT JSON_EXTRACT(@report, '$.encounter_type_uuid') INTO @encounter_type FROM mamba_dim_json;
    SELECT JSON_EXTRACT(@report, '$.concepts_locale') INTO @concepts_locale FROM mamba_dim_json;
    SELECT JSON_EXTRACT(@report, '$.table') INTO @column_array FROM mamba_dim_json;
